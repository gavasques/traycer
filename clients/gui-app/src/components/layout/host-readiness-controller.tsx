...
function unavailableFallback(
  ...
  return {
    ...
    actions: [
      {
        // The SAME coalesced registry read the directory surfaces offer, so it
        // observes the same lock: `HostDirectoryService.refresh()` folds a
        // second caller onto the request already in flight, and a card whose
        // Retry stayed live through the background poll promised a read that
        // never started.
        label: "Retry",
