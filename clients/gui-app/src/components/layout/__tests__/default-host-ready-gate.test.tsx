...
describe("<HostReadyGate />", () => {
  ...
  it("keeps the app mounted after latching for every non-ready kind", () => {
    ...
    //
    // The three relay-only no-selection states are in this list too: a
    // background registry read that answers `signed-out` (a rejected bearer,
    // which the fetch adapter deliberately does not turn into a GUI sign-out)
    // clears the remote directory, un-observes the listing and nulls the
    // selection - reaching `searching-hosts` - without the runtime having
    // ever been unbound by a real sign-out. `mobile-no-host` and
    // `choose-host` are reachable the same way (an authoritative empty
    // listing, or a deregistered selection with other hosts still listed).
    // None of the three has a full-screen exception post-latch; see the
    // cold-start-only pin below for where their full-screen surfaces still
    // apply.
  ...
  it("still full-screens the three directory kinds BEFORE the gate has ever latched", () => {
