...
/**
 * The one-line status under a tile's label, or `null` for a label-only tile.
 *
 * Deliberately narrow, to the standard the NOTE on per-tab signals in
 * `providers-settings-panel.tsx` sets: a tile says something only when the
 * answer is already ON THE WIRE in `ProviderCliState` and means exactly one
 * thing:
 *
 * - `account` - `apiKey` is the entire content of that tab.
 * - `usage` / `env` - the profile list and the override list are the tab's own
 *   contents, counted from the same array the tab renders.
 *
 * MCP, Plugins and Skills are label-only ON PURPOSE, and not for want of a
 * count. Their lists are per-SCOPE (global vs project, and for project, per
 * workspace root - see `useProviderNativeScope`), resolved from stored
 * selections plus two other queries. A number here would either have to guess a
 * scope or re-derive that resolution as a second source of truth, and "3
 * servers" that silently means a different repo than the tab shows is exactly
 * the kind of signal that NOTE forbids. A count here has to read the cached
 * list for the scope the tab itself resolved - never one invented on this side.
 */
export function providerSectionMeta(
  tab: ProviderTabKey,
  state: ProviderCliState,
): string | null {
  switch (tab) {
    case "account":
      if (!state.apiKey.configured) return "No key";
      return state.apiKey.source === "env" ? "Key from env" : "Key set";
    case "usage":
      return countLabel(state.profiles.length, "profile", "profiles");
    case "env":
      return countLabel(state.envOverrides.length, "override", "overrides");
    case "general":
    case "modelProviders":
    case "mcp":
    case "plugins":
