...
describe("directory transitions keep the app mounted", () => {
  // The three relay-only no-selection states are reachable on a LIVE
  // session, not only on a cold start: a background registry read that
  // answers `signed-out` (a rejected bearer, which the fetch adapter
  // deliberately does not turn into a GUI sign-out) clears the remote
  // directory, un-observes the listing and nulls the selection, landing on
  // one of these three kinds with the app still fully mounted. Unmounting
  // there is exactly the regression this file's helper exists to catch.
  it.each([
    { kind: "searching-hosts" as const },
    { kind: "mobile-no-host" as const },
    { kind: "choose-host" as const },
  ])(
    "keeps the SAME app node mounted and shows the directory strip on $kind",
    ({ kind }) => {
      const harness = mountSwitchSurface(
        { kind: "ready" },
        PRESENTATION,
      ...
      harness.setReadiness({ kind }, PRESENTATION);
