...
export interface DefaultHostReadinessPresentation {
   ...
   * `actions: []`.
   *
   * `refreshDirectory` is owned once by the readiness controller, for the same
   * reason `requestRespawn` is: `HostDirectoryService.refresh()` coalesces
   * callers onto one in-flight request, so every slot offering the action must
   * observe the same `directoryRefreshing` lock or one of them would show a
   * live button for a fetch that is already running.
   */
 ...
 *  - `directory` - children, plus the amber strip carrying a DIRECTORY
 *                 recovery (re-read the registry, or pick a host). The three
 *                 relay-only no-selection states - `searching-hosts`,
 *                 `mobile-no-host`, `choose-host` - live here.
 *
 * There is no full-screen row. Post-latch the gate never replaces the app, and
 * the cold-start surfaces (the wall, the searching screen, the no-host screen)
 * are reached through the un-latched path instead.
 *
 * The three directory states used to be classified full-screen on the premise
 * that an unbound relay-only shell has no app worth keeping. That premise only
 * holds before the first paint. A live session reaches all three: a background
 * registry read that answers `signed-out` (a rejected bearer, which the fetch
 * adapter deliberately does not turn into a GUI sign-out) clears the remote
 * entries, un-observes the listing and reconciles the selection to null, and
 * the runtime unbinds synchronously - so a user with editors and terminals open
 * lands on `searching-hosts`. An authoritative empty listing reaches
 * `mobile-no-host` the same way, and a deregistered selection with other hosts
 * still listed reaches `choose-host`. Unmounting there throws away exactly the
 * work the cold-start-only gate exists to protect.
 *
 * One exhaustive switch, read by the strip, so a NEW kind cannot be added
 ...
 */
export type PostLatchSurface = "app" | "switching" | "directory" | "error";
